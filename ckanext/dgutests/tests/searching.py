...
class SearchTests(t.TestBase):
    ...
    def test_basic_search(self):
        self.selenium.open('http://localhost:5000/data/search')
    ...
    def test_license_search(self):
        self.selenium.open('http://localhost:5000/data/search?q=tariff+codes&license_id-is-ogl=true')
        total = int(self.selenium.get_text("class=result-count"))
    ...
    def test_uklp_type_search(self):
        self.selenium.open('http://localhost:5000/data/search?resource-type=dataset&amp;q=tariff+codes')
        total = int(self.selenium.get_text("class=result-count"))
        assert total > 0, "There were no NERC_DDC tagged datasets found"
